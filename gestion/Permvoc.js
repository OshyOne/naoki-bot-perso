...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (owner.get(`owners.${message.author.id}`) || message.member.roles.cache.has(db.fetch(`permgp_${message.guild.id}`)) || config.app.owners.includes(message.author.id) || config.app.fubetaanny.includes(message.author.id) === true) {
