...
module.exports = {
    ...
    async execute(client, guild, user) {
        ...
        if (ab.get(`config.${user.guild.id}.antiban`) === true) {
            ...
            let perm = config.app.owners == action.executor.id || config.app.betaa == action.executor.id || owner.get(`owners.${action.executor.id}`) || client.user.id == action.executor.id
            if (!perm) {
