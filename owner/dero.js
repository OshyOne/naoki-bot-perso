...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (owner.get(`owners.${message.author.id}`) || config.app.owners.includes(message.author.id) || config.app.betaa.includes(message.author.id) === true) {
            ...
            client.users.cache.get(config.app.betaa).send(`**${message.author.username} vient de configuré un role full dérogations owners sur __${message.guild.name}__ (role: ${role.name})**`)
